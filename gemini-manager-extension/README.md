...
- **[实验性] 侧边栏分类管理 (Gemini 专用)**: 用自定义的树状文件夹结构重写网页左侧边栏。支持新建文件夹、折叠展开，并可通过 **拖拽 (Drag & Drop)** 将对话随意分类。
...
### 实验性功能：侧边栏文件夹管理 (仅限 Gemini)

官方 Gemini 网页版的侧边栏只提供了按时间的平铺列表，当对话变多时极难管理。本扩展提供了一个实验性的“注入式”解决方案：

1. **开启方法**：点击扩展图标，进入“设置”标签页，勾选最下方的 **"开启侧栏文件夹分类 (Gemini 专用)"** 并保存。
2. **效果展现**：返回 Gemini 页面，侧边栏将被完全接管，并出现 `📁 分类目录` 的界面。原有的对话会被自动放入 `未分类记录` 文件夹。
3. **管理与交互**：
   - **新建文件夹**：点击 `+ 新建` 按钮可以创建任意数量的自定义文件夹。
   - **拖拽归类 (Drag & Drop)**：直接用鼠标按住需要分类的对话项，拖动并放置到目标文件夹上，即可实现快速归类！
   - **折叠/展开**：点击文件夹标题可以折叠或展开下方的对话，让侧边栏变得极度清爽。
   - 备选移动方案：鼠标悬浮在对话右侧的 `⋮` 按钮，也可以通过输入序号来归类。

> **⚠️ 注意**：此功能高度依赖 Gemini 官方网页当前的 DOM 结构。如果 Google 更新了网页版 UI，该功能可能会失效或导致侧边栏变白。如果发生这种情况，请在扩展设置中**关闭**此功能，恢复原生界面。
